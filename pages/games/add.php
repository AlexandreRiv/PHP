...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        // Insérer les niveaux
        if (!empty($oldLevels)) {
            $stmtLvl = $db->prepare('INSERT INTO levels (game_id, name, difficulty, description) VALUES (?, ?, ?, ?)');
            foreach ($oldLevels as $lvl) {
                $diff = in_array($lvl['difficulty'], ['easy', 'medium', 'hard', 'extreme']) ? $lvl['difficulty'] : 'medium';
                $stmtLvl->execute([$gameId, $lvl['name'], $diff, $lvl['description']]);
            }
        }

        // Insérer les succès
        if (!empty($oldAchievements)) {
            $stmtAch = $db->prepare('INSERT INTO achievements (game_id, name, description, rarity) VALUES (?, ?, ?, ?)');
            foreach ($oldAchievements as $ach) {
                $rarity = in_array($ach['rarity'], ['common', 'uncommon', 'rare', 'epic', 'legendary']) ? $ach['rarity'] : 'common';
                $stmtAch->execute([$gameId, $ach['name'], $ach['description'], $rarity]);
            }
        }
    ...
    <div class="max-w-3xl mx-auto px-4 py-10">
        ...
        <form action="" method="POST" class="space-y-8">
            ...
            <div class="bg-linear-to-br from-tft-card to-tft-card-deep border border-tft-border rounded-xl p-6">
                ...
                <p class="text-gray-600 text-xs mt-3">Les succès vides seront ignorés.</p>
            </div>
